function [ ensemble_spindle_mean, ensemble_spindle_std, spindle_means, spindle_stds ]...
    = timelapse_spindle_std_stretch_only_1to2um_only( directory, threshold )
...
for n = 1:length(files)
    ...
    %limit the spindle length to 1 to 3 microns
    spindle_lt1000 = cellfun(@(x) x < 1000,spindle_length);
    spindle_gt2000 = cellfun(@(x) x > 2000,spindle_length);
    spindle_exclude = spindle_lt1000 | spindle_gt2000;
    spindle_include = ~spindle_exclude;
    %only count cells with stretched plasmid
    stretch_index = cellfun(@(x) x > threshold,aspect_ratios);
    %index which cells are whole (not split)
    split = logical(cell2mat(split));
    whole = ~split;
    include = stretch_index & whole & spindle_include;
    ...
    spindle_mean = mean(cell2mat(spindle_length),'omitnan');
    spindle_means(n) = spindle_mean;
    spindle_std = std(cell2mat(spindle_length),1,'omitnan');
    spindle_stds(n) = spindle_std;   
end

ensemble_spindle_mean = mean(spindle_means,'omitnan');
ensemble_spindle_std = mean(spindle_stds,'omitnan');
